 ...
 *      new node.  After each arrival (except the final one) the child BLOCKS
 *      until the parent sends back an acknowledgement, and only then proceeds
 *      to the next node.  They exit cleanly when done.
 *    - The PARENT reads these messages (non-blocking in the GUI loop, select()
 *      in headless mode), snaps each traveler's dot to the reported node,
 *      prints "[PID] arrived at node X | next node: Y" to the terminal, and
 *      writes a one-byte acknowledgement back so the child may continue.
 *
 *  IPC mechanism chosen: TWO anonymous pipes per child --
 *    (1) data pipe : child writes node-arrival messages, parent reads;
 *    (2) ack  pipe : parent writes a one-byte acknowledgement, child reads.
 *  The ack pipe turns each arrival into a request/response handshake so the
 *  child cannot run ahead of the parent.  Rationale:
 *    - Already the established pattern in this codebase (M4 startup barrier).
 *    - Node-arrival events are discrete and ordered; the matching ack flows
 *      strictly parent-to-child, which is exactly what pipes were designed for.
 *    - No synchronization primitives (mutexes / semaphores) required, unlike
 *      shared memory.
 *    - One pipe pair per child avoids multi-writer races and keeps each
 *      traveler's message stream independent.
 ...
 *                  GUI position snap.  The parent answers each non-final
 *                  MSG_ARRIVE with a single ack byte on the ack pipe.
 *
 *  Build (see Makefile):
...
typedef struct {
    ...
    int       ack_fd[2];       /* [0] = child read end,  [1] = parent write end */
...
static int load_input(Graph *g, const char *fn, Traveler *trav, int *numTrav) {
    ...
    for (int i = 0; i < T; i++) {
        ...
        trav[i].ack_fd[0]  = trav[i].ack_fd[1]  = -1;
    }
 ...
 *   MSG_ARRIVE -> snap dot to reported node, print terminal line, and (unless
 *                 this was the destination) send a one-byte ack back so the
 *                 child is released to walk to its next node.
        ...
        while ((r = read(trav[i].pipe_fd[0], &msg, sizeof msg)) == (ssize_t)sizeof msg) {
            ...
            if (msg.type == MSG_PATH) {
                ...
                if (msg.plen <= 1) {
                    ...
                    trav[i].pipe_fd[0] = -1;
                    if (trav[i].ack_fd[1] >= 0) {
                        close(trav[i].ack_fd[1]);
                        trav[i].ack_fd[1] = -1;
                    }
                }
            ...
            if (msg.next < 0) {
                ...
                close(trav[i].pipe_fd[0]);
                trav[i].pipe_fd[0] = -1;
                if (trav[i].ack_fd[1] >= 0) {  /* no further acks for this child */
                    close(trav[i].ack_fd[1]);
                    trav[i].ack_fd[1] = -1;
                }
                ...
                fflush(stdout);
                /* ---- send the go-ahead so the child may proceed ---- */
                if (trav[i].ack_fd[1] >= 0) {
                    char ack = 1;
                    ssize_t aw = write(trav[i].ack_fd[1], &ack, sizeof ack);
                    (void)aw;
                }
                DBG("PARENT: traveler %d at node %d -> %d (ack sent)\n",
...
static void cleanup_children(Traveler *trav, int n) {
    /* close remaining read ends first so children writing get SIGPIPE,
       and close ack write ends so any child blocked on read(ack) gets EOF */
    for (int i = 0; i < n; i++) {
...
int main(int argc, char *argv[]) {
    ...
    /* ---- create one DATA pipe and one ACK pipe per traveler BEFORE forking ---- */
    for (int i = 0; i < n; i++) {
        if (pipe(trav[i].pipe_fd) != 0) {
            perror("pipe");
            for (int j = 0; j < i; j++) {
                close(trav[j].pipe_fd[0]); close(trav[j].pipe_fd[1]);
                close(trav[j].ack_fd[0]);  close(trav[j].ack_fd[1]);
            }
            gfree(&g); return 1;
        }
        if (pipe(trav[i].ack_fd) != 0) {
            perror("pipe");
            close(trav[i].pipe_fd[0]); close(trav[i].pipe_fd[1]); /* the data pipe just made */
            for (int j = 0; j < i; j++) {
                close(trav[j].pipe_fd[0]); close(trav[j].pipe_fd[1]);
                close(trav[j].ack_fd[0]);  close(trav[j].ack_fd[1]);
            }
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (pid == 0) {
            /* ====== CHILD i ====== */
            /* Data pipes: close all read ends; close write ends of OTHER travelers.
               Ack  pipes: close all write ends (parent's); close read ends of OTHER
               travelers.  Keep only OWN data-write end and OWN ack-read end. */
            for (int j = 0; j < n; j++) {
                close(trav[j].pipe_fd[0]);
                if (j != i) close(trav[j].pipe_fd[1]);
                close(trav[j].ack_fd[1]);
                if (j != i) close(trav[j].ack_fd[0]);
            }
            int wfd = trav[i].pipe_fd[1];   /* own data write end */
            int rfd = trav[i].ack_fd[0];    /* own ack  read  end */
            ...
            /* Walk the path: sleep per edge, report each node arrival, then
               BLOCK until the parent acknowledges before moving on. */
            for (int step = 1; step < plen; step++) {
                ...
                /* ---------- HANDSHAKE ----------
                 * Unless we are at the destination (no next node), wait for the
                 * parent's one-byte acknowledgement before leaving this node. */
                if (am.next >= 0) {
                    char ack;
                    ssize_t ar;
                    do {
                        ar = read(rfd, &ack, sizeof ack);
                    } while (ar < 0 && errno == EINTR);   /* retry on signal */
                    if (ar <= 0)
                        break;   /* parent closed the ack pipe -> stop walking */
    ...
    /* Parent closes the ends it does not use:
       - data write ends (it only reads data),
       - ack  read  ends (it only writes acks). */
    for (int i = 0; i < n; i++) {
        close(trav[i].pipe_fd[1]);
        trav[i].pipe_fd[1] = -1;
        close(trav[i].ack_fd[0]);
        trav[i].ack_fd[0] = -1;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (r == (ssize_t)sizeof pm && pm.type == MSG_PATH) {
            ...
            if (pm.plen <= 1) {
                ...
                close(trav[i].pipe_fd[0]);
                trav[i].pipe_fd[0] = -1;
                if (trav[i].ack_fd[1] >= 0) { close(trav[i].ack_fd[1]); trav[i].ack_fd[1] = -1; }
            }
